...
set rc [catch {
  open_project {BAK_DEMO.prj}
  import_device ef2_1.db -package EF2L15LG100B
  elaborate -top {SGPIO_DEMO_TOP}
  read_adc ../../BAK_PIN.adc
  export_db {BAK_DEMO_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  optimize_rtl
  write_verilog simulation/BAK_DEMO_rtl_sim.v
  report_area -file BAK_DEMO_rtl.area
  export_db {BAK_DEMO_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  ...
  optimize_gate -packsim simulation/BAK_DEMO_gate_sim.v -maparea BAK_DEMO_gate.area
  legalize_phy_inst
  export_db {BAK_DEMO_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
